...
class TestView(TestCase):
    ...
    def test_post_detail(self):
        ...
        # 첫 번째 포스트의 작성자가(author)가 포스트 영역에 있다
    ...
    def test_tag_page(self):
        ...
        self. assertIn(self.tag_hello.name, soup.h1.text) # 이페이지에서는 h1택가 한번만 나오기떄문에 여기서 확인
    ...
    def test_create_post(self):
        ...
        # staff가 아닌 trump로 로그인한다
        self.client.login(username='trump', password='somepassword')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff 인 obama로 로그인한다
    ...
    def test_comment_form(self):
        ...
        comment_area = soup.find('div', id='comment-area')
        new_comment_div: Union[Tag, NavigableString, None, int] = comment_area.find('div', id=f'comment={new_comment.pk}')
